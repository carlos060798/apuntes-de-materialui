  ...
  try {
    ...
    if (channel) {
      ...
      io.to(channelId).emit("newChannelMessage", {
        ...
        content: newMessage.content, // Asegúrate de que aquí también se use 'content'
...
export const registerSocketServer = (server: http.Server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("joinChannel", (channelId) => {
      socket.join(channelId);
      emitChatHistory(socket, channelId); // Enviar historial de chat
    });
    ...
    socket.on("disconnectFromChat", ({ channelId }) => {
